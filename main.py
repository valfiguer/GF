...
def process_single_article(item, repo, config, logger) -> bool:
    ...
    try:
        ...
        # Generate web article (before Telegram publish to get the slug)
        web_slug = None
        if config.web.enabled:
            try:
                from web.article_pipeline import create_web_article_from_item
                web_id = create_web_article_from_item(item)
                if web_id:
                    from db import get_repository as _get_repo
                    web_art = _get_repo().get_web_article_by_article_id(item.article_id)
                    web_slug = web_art['slug'] if web_art else None
            except Exception as e:
                logger.error(f"Error creating web article: {e}")
...
def main():
    ...
    # Start web server in-process if configured
    if os.getenv("WEB_IN_PROCESS", "").lower() in ("true", "1", "yes"):
        try:
            import threading
            import uvicorn
            ...
            def _run_web():
                uvicorn.run(web_app, host=config.web.host, port=config.web.port, log_level="warning")

            web_thread = threading.Thread(target=_run_web, daemon=True)
            web_thread.start()
            logger.info(f"🌐 Web server started at {config.web.base_url}")
        except Exception as e:
            logger.warning(f"Could not start in-process web server: {e}")

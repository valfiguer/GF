...
class TelegramPublisher:
    ...
    def _create_source_keyboard(
        ...
        web_article_slug: Optional[str] = None
    ) -> InlineKeyboardMarkup:
        """
        Create inline keyboard with source link.

        If web_article_slug is provided, the primary button links to the
        GoalFeed portal and the source becomes a secondary button.

        Args:
            source_url: URL to the source article
            source_name: Name of the source
            web_article_slug: Slug for the web portal article
        ...
        if web_article_slug:
            config = get_config()
            portal_url = f"{config.web.base_url}/article/{web_article_slug}"
            buttons.append([InlineKeyboardButton(text="📰 Leer en GoalFeed", url=portal_url)])
            # Secondary button: original source
            src_text = f"📖 Fuente: {source_name}" if source_name else "📖 Fuente original"
            buttons.append([InlineKeyboardButton(text=src_text, url=source_url)])
        else:
            button_text = f"📖 Leer en {source_name}" if source_name else "📖 Leer fuente"
            buttons.append([InlineKeyboardButton(text=button_text, url=source_url)])
    ...
    async def send_photo_async(
        ...
        Args:
            ...
            source_name: Name of the source
            web_article_slug: Slug for web portal article link
        ...
        try:
            for attempt in range(self.max_retries):
                try:
                    ...
                    reply_markup = None
                    if source_url:
                        reply_markup = self._create_source_keyboard(source_url, source_name, web_article_slug)
    ...
    def send_photo(
        ...
        try:
            return loop.run_until_complete(
                self.send_photo_async(image_data, caption, source_url, source_name, web_article_slug)
            )
        finally:
            loop.close()
...
async def publish_article_async(
    ...
    return await publisher.send_photo_async(
        image_data, caption, source_url, source_name, web_article_slug
    )
